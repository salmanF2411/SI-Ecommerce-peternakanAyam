    ...
    <body class="<?= $user && $user['role'] !== 'customer' ? 'staff-page' : 'customer-page' ?>">
        <?php if ($user && $user['role'] !== 'customer'): ?>
            <aside class="sidebar">
                ...
                <nav>
                    <a class="<?= $page === 'dashboard' && $adminTab === 'overview' ? 'active' : '' ?>" href="?page=dashboard">⌂
                        Dashboard</a>
                    <a class="<?= $page === 'dashboard' && $adminTab === 'products' ? 'active' : '' ?>"
                        href="?page=dashboard&tab=products">▦ Produk</a>
                    <a class="<?= $page === 'dashboard' && $adminTab === 'orders' ? 'active' : '' ?>"
                        href="?page=dashboard&tab=orders">◷ Pesanan</a>
                    <a class="<?= $page === 'dashboard' && $adminTab === 'reports' ? 'active' : '' ?>"
                        href="?page=dashboard&tab=reports">▤ Laporan</a>
                    <a class="<?= $page === 'dashboard' && $adminTab === 'logs' ? 'active' : '' ?>"
                        href="?page=dashboard&tab=logs">◌ Aktivitas</a>
                </nav>
                <form method="post"><input type="hidden" name="action" value="logout"><button class="nav-logout">Keluar</button>
                </form>
            </aside>
            <main class="app-main">
                ...
                <header class="site-header">
                    ...
                    <nav>
                        ...
                        <?php if ($user): ?>
                            ...
                            <form method="post"><input type="hidden" name="action" value="logout"><button
                                    class="link-button">Keluar</button></form>
                        <?php else: ?>
                            <a href="?page=login">Masuk</a>
                        <?php endif; ?>
                        <a class="cart-link <?= $page === 'cart' ? 'active' : '' ?>" href="?page=cart">🛒
    ...
    if ($page === 'home'): ?>
                <section class="hero">
                    <div class="hero-copy"><span class="eyebrow">DARI KANDANG, UNTUK KELUARGA</span>
                        ...
                        <p>Ayam, telur, dan pupuk pilihan dari Rumah Pitik. Dikelola dengan perhatian, dikirim untuk kebutuhan
                            sehari-hari.</p><a class="button primary" href="?page=products">Jelajahi produk <span>→</span></a>
                    </div>
                    <div class="hero-image"><img
                            src="https://images.unsplash.com/photo-1548550023-2bdb3c5beed7?auto=format&fit=crop&w=1200&q=85"
                            alt="Peternakan ayam"></div>
                </section>
                <section class="intro" id="about">
                    <div><span class="eyebrow">KENAPA RUMAH PITIK</span>
                        <h2>Yang baik dimulai dari cara yang baik.</h2>
                    </div>
                    <p>Kami merawat produk dari sumbernya agar sampai di meja Anda dalam kondisi terbaik. Sederhana, jujur, dan
                        konsisten.</p>
                </section>
                <section class="category-grid">
                    ...
                    <div class="category-items"><?php foreach ($categories as $cat): ?><a
                                href="?page=products&category=<?= e($cat['slug']) ?>"><span><?= $cat['name'] === 'Pupuk' ? '✦' : ($cat['name'] === 'Telur' ? '◒' : '♧') ?></span><b><?= e($cat['name']) ?></b><small>Jelajahi
                                    pilihan →</small></a><?php endforeach; ?></div>
                </section>
                ...
                <section class="catalog-head"><span class="eyebrow">KATALOG PRODUK</span>
                    <h1>Temukan yang Anda<br><em>butuhkan hari ini.</em></h1>
                    <form class="search-form"><input name="search" value="<?= e($search) ?>"
                            placeholder="Cari ayam, telur, pupuk..."><input type="hidden" name="page" value="products"><button
                            class="button dark">Cari</button></form>
                </section>
                <div class="filter-row"><a class="<?= !$category ? 'active' : '' ?>"
                        href="?page=products">Semua</a><?php foreach ($categories as $cat): ?><a
                            class="<?= $category === $cat['slug'] ? 'active' : '' ?>"
                            href="?page=products&category=<?= e($cat['slug']) ?>"><?= e($cat['name']) ?></a><?php endforeach; ?>
                </div>
    ...
    ?>
            <section class="detail">
                ...
                <div class="detail-copy">
                    ...
                    <strong class="price-large"><?= money($product['price']) ?> <small>/
                            <?= e($product['unit']) ?></small></strong>
                    ...
                    <?php if ($product['stock']): ?>
                        <form method="post" class="buy-form js-buy-form">
                            ...
                            <div class="detail-qty-control">
                                <button type="button" class="btn-qty-detail" data-action="minus"
                                    aria-label="Kurang">&minus;</button>
                                <input type="number" name="quantity" class="qty-detail-input" value="1" min="1"
...
if ($page === 'cart' || $page === 'checkout' || $page === 'orders') {
    ...
    if ($page === 'orders') {
        ...
        layoutStart('Pesanan Saya', $user); ?>
                <section class="content-head"><span class="eyebrow">AKUN PELANGGAN</span>
                    <h1>Pesanan saya</h1>
                    <p>Pantau perjalanan pesanan Anda dari sini.</p>
                </section>
                <div class="order-list"><?php foreach ($orders as $order): ?>
                        <article class="order-row">
                            <div>
                                <b><?= e($order['order_code']) ?></b><small><?= date('d M Y, H:i', strtotime($order['created_at'])) ?></small>
                            </div><strong><?= money($order['total']) ?></strong><span
                                class="status status-badge <?= statusClass($order['status']) ?> <?= strtolower(trim($order['status'])) ?>"><?= e($order['status']) ?></span>
                        </article>
                    <?php endforeach;
                if (!$orders): ?>
                        <div class="empty">Belum ada pesanan. <a href="?page=products">Mulai belanja →</a></div><?php endif; ?>
                </div>
                <?php layoutEnd($user);
                exit;
    }
    $items = cartItems();
    layoutStart($page === 'cart' ? 'Keranjang' : 'Checkout', $user); ?>
            <section class="content-head"><span
                    class="eyebrow"><?= $page === 'cart' ? 'KERANJANG BELANJA' : 'LANGKAH TERAKHIR' ?></span>
                ...
                <div class="cart-layout" id="cartLayout">
                            ...
                            <?php foreach ($items as $item): ?>
                                <article class="cart-item" id="cart-row-<?= $item['product']['id'] ?>">
                                    <div class="cart-item-img">
                                        <img src="<?= e(productImage($item['product']['image'])) ?>"
                                    ...
                                    <div class="cart-item-info">
                                        ...
                                        <div class="cart-item-meta">
                                            ...
                                            <span class="cart-unit-price"><?= money($item['product']['price']) ?> /
                                                <?= e($item['product']['unit']) ?></span>
                                        </div>
                                        <div class="cart-item-stock">Stok: <?= (int) $item['product']['stock'] ?>
                                            <?= e($item['product']['unit']) ?></div>
                                    </div>
                                    ...
                                    <div class="cart-item-subtotal">
                                        ...
                                        <strong class="cart-subtotal-value"
                                            id="subtotal-<?= $item['product']['id'] ?>"><?= money($item['subtotal']) ?></strong>
                                    </div>
                    ...
                    <aside class="summary" id="cartSummary">
                        ...
                        <a href="?page=checkout" class="button primary wide<?= !$items ? ' disabled-btn' : '' ?>"
                            id="btnCheckout" <?= !$items ? ' aria-disabled="true" tabindex="-1"' : '' ?>>
                            Lanjut ke Checkout &#8594;
                        </a>
            ...
            <?php else: ?>
                <form method="post" class="checkout-layout"><input type="hidden" name="action" value="checkout">
                    <div class="checkout-form"><label>Nama penerima<input required name="customer_name"
                                value="<?= e($user['name']) ?>"></label><label>Nomor telepon<input required name="phone"
                                value="<?= e($user['phone']) ?>"></label><label>Alamat pengiriman<textarea required
                                name="address" rows="4"><?= e($user['address']) ?></textarea></label><label>Metode
                            pembayaran<select name="payment_method">
                                <option>Transfer bank</option>
                                <option>Cash on delivery</option>
                                <option>Bayar di tempat</option>
                            </select></label><label>Catatan <small>(opsional)</small><textarea name="notes" rows="3"
                                placeholder="Contoh: kirim pagi hari"></textarea></label><button class="button primary">Buat
                            pesanan &#8594;</button></div>
                    <aside class="summary">
                        <h3>Pesanan Anda</h3><?php foreach ($items as $item): ?>
                            <div><span><?= $item['quantity'] ?> &times;
                                    <?= e($item['product']['name']) ?></span><b><?= money($item['subtotal']) ?></b></div>
                        <?php endforeach; ?>
                        <hr>

...
<div class="dash-head">
    ...
    <?php if ($user['role'] === 'admin'): ?>
        <button class="btn btn-primary"
            onclick="document.getElementById('form-stok').style.display='block';this.style.display='none'">
...
<?php if ($user['role'] === 'admin'): ?>
    <div class="panel" id="form-stok" style="display:none;margin-bottom:24px">
        <div class="panel-header">
            ...
            <button onclick="this.closest('#form-stok').style.display='none'"
                style="background:none;color:var(--muted);font-size:13px">
            ...
            <div class="form-group" style="margin:0">
                ...
                <select class="form-control" name="product_id" required>
                    ...
                    <?php foreach ($products as $p): ?>
                        <option value="<?= $p['id'] ?>"><?= e($p['name']) ?> (Stok: <?= $p['stock'] ?>         <?= e($p['unit']) ?>)
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <tbody>
                <?php foreach ($products as $i => $item): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <b
                                style="font-size:16px;<?= $item['stock'] <= 10 ? 'color:var(--red)' : 'color:var(--green)' ?>">
                                <?= $item['stock'] ?>
                            </b>
                        </td>
                        <td><?= e($item['unit']) ?></td>
            ...
            <tbody>
                <?php foreach ($stockHistory as $row): ?>
                    <tr>
                        ...
                        <td>
                            <span
                                class="status-badge <?= $row['type'] === 'Masuk' ? 'status-diproses' : ($row['type'] === 'Keluar' ? 'status-menunggu' : 'status-dikonfirmasi') ?>">
                                <?= e($row['type']) ?>
                            </span>
                        </td>
